...
其实是最近捏了一堆尼采大战Typescript，习近平大战毛泽东什么的。  
...
*   **实现：** 计算用户输入与 Lorebook 条目的**余弦相似度 (Cosine Similarity)**。如果相似度超过阈值（比如 0.8），就算没有出现关键词，也会把设定扔进去。
