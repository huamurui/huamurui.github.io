...
算了在这里记一下 GPU 图形编程（GLSL）：通过操纵简单的数值，再借助 gpu 强大的并行计算，来处理 3d，材质等图形特效。  以及，在希望处理图形之外的大量矩阵运算...  会有类似的但另外的东西。
但回到图像，大概，有，两种 shader，VERT 和 FRAG，前者和 3d 视角变换很相关，但这里的雨滴，更多是一种颜色、材质的模拟，主要用的是后面这个。  

1. 水滴，有一个弧形，这里用一个纹理图片来描述。
2. 根据水滴在后景的位置，和某个点在水滴上的位置，去取背后图片的颜色，也就是环境色
3. 再结合一个水滴高光的材质模拟的计算
4. 最后把算的颜色涂到屏幕上去

关于 GLSL 等运行在 gpu 上的编程，一段 GLSL 代码，GPU 会把它复制几万份，分发给所有小运算单元，让他们在同一瞬间、并行地计算出所有像素的颜色。许多时候不会有循环之类的东西，而是直接就很... 声明式？    
还有一件事是， intel, snapdragon 等等芯片说的 gpu 个数就是个数...  nvdia 标注产品喜欢说运算单元，也就是他们 cuda 的个数。  
