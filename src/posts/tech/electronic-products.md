...
- UART, I2C, SPI
  ...
  - I2C 
    ...
    - 关于总线，一个常见场景是，一个 i2c 上连了一堆传感器，处理器隔一段时间扫一遍拿一遍数据。这时候处理器是主，其他是从，不容易冲突，并且接线简单。
    - 100 kHz , 400 kHz , 3.4 mHz(需要特殊驱动和上拉)
    - 100 - 400kbps
  - SPI
    - 同步、全双工、高速。通常需要四根线：MOSI MISO SCLK CS/SS(Chip Select / Slave Select)
    - SPI 有时钟线，有主从，两条数据线独立收发，全双工。收消息是这样干的，主设备发送命令去让从设备发送信息然后读取，时间信号也还是主设备提供(感觉控制欲好强)，而多出的 CS/CC 则像一个专门的指挥棒或点名器，一次选一个从设备通信，让一堆从设备不会挤占 MOSI MISO。
    - 10 - 100 MHz
    - 10 - 100+ Mbps
  - 其他
    - SPI 比上两个流量大，一个是两条数据线比 i2c 强，一个是带时钟线频率可以更高比 uart 强。但也不止如此， spi 只看频率也还是比 i2c 高了太多， i2c 为了在一个时钟和一条数据线上搞多主多需要的操作多了一些，而 spi 确定只有一个主，片选单开一条线，剩下的两条数据线完全为了速度。一些分辨率不是太高的屏幕，还有 sd 卡会用到 SPI。
...
- 姿态传感器做了个可以流动的水的模拟，似乎 i2c 和 spi 模式都能用，但普通传感器，i2c 连一串更简单些。
......
...
2. 
我才发现 esp32 开发板的引脚给的太万能了，几乎每个脚都能是 gpio/adc/rtc... 中的任意一种. luckfox rv1106 上面... 甚至有好多跨脚冲突的，这个用了那个就不能用的x
...
- 姿态, 加速度传感器
  ...
  - 姿态数据融合处理应用
    - 传感器校准
    - 滤波-多传感器(加速度|陀螺仪)互补/Mahony/卡尔曼滤波
    - 坐标系转换->导航
    - 步态检测、手势识别
...
原来主板叫 mother board。妈妈板。 || 大家都要插在妈妈上 ||

1. 找元件 ->
2. 绘制原理图 -> 设计PCB -> 
3. 打样，焊接...

里面的 `2` 会是在 EDA/CAD 上操作的主要部分...  
我去看了几眼嘉立创的那个软件，然后...  
发现我现在需要看的能看的不是软件操作，而应该去补补基础的电路知识...  
...
1. 电容：不管哪里的电容都可以看作一个小蓄电池，但具体规格不同，作用也会非常不同。
  - 核心：阻抗随频率变化，它对不同频率的电流呈现出完全不同的“性格”。
    - 公式：容抗 Xc = 1 / (2πfC)。
    - 这意味着：频率f越高，容抗Xc越小，电流越容易通过；对于直流(f=0)，容抗无穷大，完全阻断。
  - 瓷片电容 MLCC（小容量高频去耦）、电解电容（大容量储能）、钽电容（中等容量、中频滤波和储能）。
2. 电阻：“限流”、“分压”和“阻抗匹配”
  - 上拉/下拉电阻的本质，是为悬空或开集输出的引脚，在不被主动驱动时，提供一个确定的电平状态（高或低），防止其“悬空”受干扰。
  - 碳膜、金属膜、贴片排阻。
3. 电感：... 功率电感（用于DC-DC，储能）、磁珠（用于滤波）。
4. 0Ω 电阻 debug； 二极管，保险丝，保护电路...

芯片内部...是一样的吗？  
不，集成电路内部几乎全是晶体管，纯粹的为逻辑编码实现，模拟信号处理等。而上面提到的电路和元件，更多的是，物理、电气层面的连接与支持。  
或许，计算这件事其实算是这些硬件的一种核心，也只有它们值得，能够使用集成电路/晶体管的方式更加高效更加微观的实现。  
而将这些东西微观的高效的实现了之后，也许进一步，那些必不可少的外围支持才也可以小一圈，氮化镓充电器就是如此，一个更高效的小芯片，省掉一大坨支持元件。  
而关于集成电路/晶体管... ↓  不过把先它们当黑箱，用更抽象的概念去使用理解，也不是不行。  
#### 还有一件事
